...
class SensorTowerAdvancedApiService {
     ...
     */
    private async request(url: string, errorMessage: string): Promise<any> {
        if (!AUTH_TOKEN) {
...
server.tool(
    "get_demographics",
    "Fetches user demographics (age, gender) for specific apps over a time range. Useful for user persona analysis and marketing targeting.",
    {
    ...
    async ({ os, app_ids, start_date, end_date, countries, date_granularity }) => {
        try {
            ...
            const data = await apiService.fetchDemographics(
                os,
                appIdArray,
                start_date,
                end_date,
